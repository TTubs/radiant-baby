...
namespace Gamekit2D
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        {
            m_StartingPosition = checkpoint.transform.position;
        }
        ...
        {
            currentPower += amt;
            if (currentPower > maxPower && damageable.CurrentHealth < damageable.startingHealth)
            {
                damageable.GainHealth(1);
            }
            currentPower = Mathf.Clamp(currentPower, 0, maxPower);
            m_ShotSpawnGap = 1f / (currentPower / 5 + 3);
        }
